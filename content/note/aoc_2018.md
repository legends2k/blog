...
This is the start of puzzles where solutions can get stuck in infinite loops without a deterministic answer.  I’d bail out of a loop if solutions didn’t change across iterations.
...
I implemented BFS-based pathing.  The puzzle itself wasn’t hard (skill-wise) but involved a lot of busy work following complicated rules meticulously. Though I struggled quite a bit, I’m proud I solved it without help.
...
Part 2 of the puzzle required running the simulation for 1 billion times.  This had to be short-circuited to get a result (in reasonable time).  Printing the values showed a repeating series; I vaguely remember cheating for this.
...
Shortest distance path finding problem whose input is a very long [regular expression][regex].  I’m glad I didn’t resort to any regex libraries as it’s fairly easy to parse _by hand_.  I implemented BFS for the shortest path reusing code from day 15.  Realized many solutions were a lot simpler than mine; no path finding at all! 😲
